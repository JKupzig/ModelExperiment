...
set_names <- c("reservoir algorithm (V1)", "reservoir algorithm (V2)")
...
for (name in set_names) {
  ...
  data_to_plot %>%
    ...
    theme_bw() +
    theme(
      legend.position = legend_position, # 0.1, 0.1 -> res1 | 0.1, 0.2 --> res2
...
for (col in c("h", "l")) {
  ...
  plot(ref[[col_x]], ref[[col_y]],
    xlim = c(0, max_xy), ylim = c(0, max_xy), main = sprintf("res1: %s", col),
