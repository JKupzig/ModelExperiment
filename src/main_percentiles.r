...
for (model in names(comparison_models)){
  ...
  #delta_to_models[run,,][ref_data == 0 & delta_to_models[run,,] == 0] <- NA
...
data_to_plot <- delta_long %>%
  ...
  dplyr::filter(! (Var2 %in% c("mean", "sd", "pearson"))) %>%
...
na_data_to_plot <- data_to_plot[rowSums(is.na(data_to_plot)) > 0,]
length(unique(na_data_to_plot$Var3)) #35 basins with na value
...
data_to_plot %>%
  ...
  ggplot(., aes(x=Var1, y = Freq)) +
    ...
    geom_jitter(aes(y = Freq), col="gray20",
                alpha = 0.3, show.legend = F, shape=4, size=1., stroke=1.5) +
...
ggplot(data_to_plot, aes(x=Var3, y = Freq, group=Var2, col=coloring)) +
geom_point() +
theme_bw() +
theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
facet_grid(plot_type ~ Var1, scales = "free_y") +
scale_color_manual("type", values=hcl.colors(5, "viridis")) +
...
ggsave(plot_name, units="cm", width=30, height=20, dpi=300)
...
delta_long %>%
  filter(Var1 == "schneider_reservoirs" | Var1 == "hanasaki_reservoirs") %>%
  filter(Var2 == "dur_l") %>%
  filter(Freq > 2)
...
merken <- delta_long %>%
  filter(Var1 == "variable_flowvelocity") %>%
  filter(Var2 == "monthly_nse")
max(merken$Freq)
min(merken$Freq)
boxplot(merken$Freq)

nrow(merken[abs(merken$Freq) < 0.01,])/nrow(merken)
nrow(merken[(merken$Freq) > 0.0,])/nrow(merken)

nrow(merken[(merken$Freq) > 0.0,]) / nrow(merken)

nrow(merken[(merken$Freq) < 0.0,]) / nrow(merken)
nrow(merken[(merken$Freq) < -0.01,]) / nrow(merken)
