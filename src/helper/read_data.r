...
read_kge_and_define_good_basins <- function(min_kge = 0.4, max_kge = NULL){
  ...
  for (file_to_read in files_to_read){
    cal_result <- read.table(file.path(ROOT, file_to_read), sep="\t", header=T)
    cal_result$variant <- substr(file_to_read, 23, 40)
    all_kges <- rbind(all_kges,  cal_result)
  }

  basins <- unique(all_kges$station[all_kges$KGE_cal >= min_kge])
  basins_to_return <- c()
  if (!is.null(max_kge)){
    for (basin in basins){
    ...
    basins_to_return <- basins
  }

  return(list("behavioural"= basins_to_return, "data"= all_kges))
}


read_benchmarks_all <- function(column = "kge_val"){
  FOLDER <- "./data/evaluation"
  #ROOT <- file.path(FOLDER, "cal_result_benchmarks_model_m%i.txt")
  ...
  #   cal_results[[sprintf("model_m%i", model)]] <- model_result[[column]]
  # }
...
read_benchmarks_uncertainty <- function(column = "kge_val", uncertainty_type = 1){
  ...
  COLUMN_1 <- sprintf("uncertainty%i_wetlStorage100", uncertainty_type)
  COLUMN_2 <- sprintf("uncertainty%i", uncertainty_type)
  results <- NULL
  models <- seq(8,19,1)
  for (model_type in models){
     ...
     idx_dim_3 <- which(dimnames(cal_results)[[3]] %in% c(COLUMN_1, COLUMN_2))

     cal_results_wanted <- cal_results[idx_dim_1, idx_dim_2,idx_dim_3,]
     cal_results_wanted_df <- as.data.frame(t(cal_results_wanted))
     names(cal_results_wanted_df) <- c(sprintf("model_m%i", model_type),
                                       sprintf("model_m%i_100", model_type))
