...
for (name in set_names) {
  ...
  for (entry in seq_along(to_analyse)) {
    ...
    # mean plot
    plot(obs_values$mean, ref_values$mean,
      main = "mean", log = "xy", pch = 19, col = scales::alpha("black", 0.8),
    ...
    # sd plot
    plot(obs_values$sd, ref_values$sd,
      main = "sd", log = "xy", pch = 19, col = scales::alpha("black", 0.8),
      ylab = "simulation", xlab = "observation"
    )
    ...
    sim_data_si <- sim_data[
      dimnames(sim_data)[[1]] %in% sprintf("%i_wetlStorage100", m1),
    ...
    for (col in si_names) {
      ...
      plot(obs, ref,
        main = col, log = "xy", pch = 19, col = scales::alpha("black", 0.8),
        ylab = "simulation", xlab = "observation"
      )
