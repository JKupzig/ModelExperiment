...
interval_mgn_h_labels <- c(
  "<-2", "-2 to -0.5", "-0.5 to -0.2", "-0.2 to -0.01",
...
summarized_values$si <- factor(
  ...
  levels = columns,
  labels = c(
    "Delta~timing~(Delta~Pearson~r)~'[-]'", "Delta~flow~volume~(Delta~alpha)~'[-]'", "Delta~variability~(Delta~beta)~'[-]'",
