...
plot_name <- sprintf(PLOT_PATTERN, "probe")
...
outlier_in_reservoir1 <- which(
  sensitive_basins$Var1 == "reservoir1" &
  sensitive_basins$Var2 == "mean" &
    sensitive_basins$Freq < -0.6)

sensitive_basins[outlier_in_reservoir1,]
sensitive_basins <- sensitive_basins[-c(outlier_in_reservoir1),]

outlier_in_reservoir2 <- which(
  sensitive_basins$Var1 == "reservoir" &
    sensitive_basins$Var2 == "mean" &
    sensitive_basins$Freq > 0.3)

sensitive_basins[outlier_in_reservoir2,]
sensitive_basins <- sensitive_basins[-c(outlier_in_reservoir2),]

outlier_in_reservoir3 <- which(
  sensitive_basins$Var1 == "reservoir" &
    sensitive_basins$Var2 == "sd" &
    sensitive_basins$Freq > 0.3)

sensitive_basins[outlier_in_reservoir3,]
sensitive_basins <- sensitive_basins[-c(outlier_in_reservoir3),]
...
sensitive_basins %>%
  ...
  geom_boxplot() +
  theme_bw() +
  facet_wrap(Var1~Var2, scale="free_y", nrow=4,
             labeller = labeller(Var2 = benchmark.labs)) +
  ylab("Value") +
