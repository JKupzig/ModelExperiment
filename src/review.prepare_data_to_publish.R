...
# delete unused metrics
not_used_columns <- c(
  "monthly_mean", "monthly_pearson", "monthly_sd", "monthly_kge",
  "mgn_a"
)
data_in_reduced <- data_in_reduced[, !dimnames(data_in_reduced)[[2]] %in% not_used_columns, ]
...
kge_info$layer <- sapply(kge_info$station, function(x) {
...
basin_sets$layer <- sapply(basin_sets$basins, function(x) {
  as.integer(strsplit(x, "X")[[1]][2])
})
...
column_names <- c(
  "grdc-id", "basin-set", "reservoir", "capacity_km3", "surface area_km2",
...
write.table(result, "./data/reservoir_data.csv",
  sep = "\t",
...
for (basin_name in all_basins$behavioural) {
  ...
  correlation <- ccf(
    ...
    na.action = na.pass, lag.max = 10, plot = FALSE
  )[1:10]
...
plot(evaluate$mean_precipitation_as_snow, evaluate$max_cor,
  pch = 20,
...
for (station in 1:330) {
  ...
  ref_station_data <- data.frame(
    Date = ref_simulated_data$date,
  ...
  magnitude_lowflow <- WaterGAPLite::Q.calcSIComplete(
    df = obs_data_val,
    func_name = "Q.__calc_mgn_l_1__"
  ) # #5th percentile Addor 2018 m3/s
  ...
  magnitude_highflow <- WaterGAPLite::Q.calcSIComplete(
    df = obs_data_val,
    func_name = "Q.__calc_mgn_h_1_adopted__"
  ) # 99th (former 95th) percentile Addor 2018
  ...
  ) # Frequency of low-flow days (<0.2 times the mean daily flow) Addor 2018
  ...
  duration_lowflow <- WaterGAPLite::Q.calcSIComplete(
    ...
    add_args = list(
      "discharge" = ref_station_data$Sim,
      "minimal_length" = 7
    )
  )
...
dimnames(result_array) <- list(
  "obs",
  c(
    "mgn_l", "mgn_a", "mgn_h", "frq_l", "frq_h", "dur_l", "dur_h",
