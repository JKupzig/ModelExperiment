...
additional <- FALSE
PLOT_PATTERN <- "./plots/supplement/supplement_s1_%s_%s_%s.png"
...
for (column in c("NSE_val", "KGE_val")) { #"logNSE_val", "KGE_mod_val",
  print(column)
  ...
  kge_info <- kge_info[kge_info$station %in% behavioural_basins$behavioural, ]

  for (group_name in names(groups)) {
    print(group_name)

    to_analyse <- groups[[group_name]]
    target_plot <- sprintf(PLOT_PATTERN, column, name, group_name)
